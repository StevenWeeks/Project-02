...
// lots of variables and some appending/creating elements for rest of script
...
let numberStudents = document.getElementsByClassName('student-item cf')
var perPage = 10
const pageButtons = document.querySelector('.pagination')
const buttonsUl = pageButtons.querySelector('ul')
let pages = Math.ceil(numberStudents.length / perPage);


function showThem (page, numberStudents) {  //shows the first 10 students on page load
  let max = (page+1) * 10;
  let min = max - 10;
  for (let i = 0; i < numberStudents.length; i++) {
    if (i >= min && i < max) { //the range to show
      numberStudents[i].style.display = 'block'; //has the display show
    } else {
      numberStudents[i].style.display = 'none'; //turns display off for students out of range

}
}
};
showThem(0,numberStudents) //activate the function and pass through 0 so it knows to show page 1
...
for (let i = 0; i < pager.length; i++){  //for loop using array length
  pager[i].addEventListener('click', () => {  //when an 'a' is clicked do
    showThem(i, numberStudents);  //pasases in the array spot into the previous function
  })
}
};
showRest()  //calls the function to work
